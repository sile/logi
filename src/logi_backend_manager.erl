...
%% Exported Functions
%%------------------------------------------------------------------------------------------------------------------------
%% @doc Starts a named manager process
-spec start_link(atom()) -> {ok, pid()} | {error, Reason} when
      Reason :: {already_started, pid()} | term().
start_link(Name) ->
    gen_server:start_link({local, Name}, ?MODULE, [Name], []).

%% @doc Gets the ID of the process
-spec get_id(pid()) -> logi:logger().
get_id(ManagerPid) when is_pid(ManagerPid) ->
    gen_server:call(ManagerPid, get_id).
