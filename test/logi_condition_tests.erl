...
make_test_() ->
    [
     ...
     %% {"条件指定には logi_condition:spec() 型の値が指定可能",
     %%  fun () ->
     %%          %% OK
...
get_test_() ->
    [
     ...
     {"正規化された条件指定が取得できる",
      fun () ->
              ?assertEqual([info,notice,warning,error,critical,alert,emergency], logi_condition:get_normalized_spec(logi_condition:make(info))),
              ?assertEqual([info], logi_condition:get_normalized_spec(logi_condition:make({info,info}))),
              ?assertEqual([info], logi_condition:get_normalized_spec(logi_condition:make([info])))
      end}
    ].
